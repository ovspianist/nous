...
- **Input**: `ButtonState` carries the current mask, a compatibility `pressed_latch`, and ordered `press_history`; ESP32 samples/debounces every 5ms and screens implement any hold acceleration they need. `Application::process_button_actions_()` owns configurable tap/hold gestures. A mapped navigation hold is hidden from the active screen until release (normal tap) or the 700ms threshold (configured action), preventing both actions from firing. Power-button awake tap/hold mappings are independent of the pre-boot long-hold wake check in `platforms/esp32/main.cpp`. Defaults are power tap=next page, power hold=sleep, side up/down holds=rotate CCW/CW, bottom left/right holds=font smaller/larger, and bottom Back/OK holds=None. Physical navigation defaults are side top=previous, bottom right=next, and menu right=down. All mappings are persisted in the main settings file and edited under Settings → Control.
- **Fork UI font profile**: `display/ui_font_profile.h` defaults menu, settings, and book-list text to the preserved pre-2.2.0 Terminus assets under `display/ui_fonts/legacy/`. Unitreign's Inter headers remain untouched; define `NOUS_USE_UNITREIGN_UI_FONT` to opt back into them. Keep this indirection when merging upstream font refreshes.
- **Sleep wallpapers**: An empty `sleep_image_path_` selects the user-facing **Auto Rotate** mode. `Application::do_sleep_()` advances and persists `sleep_image_idx_` so each sleep uses the next SD-card image; an individual image path pins that wallpaper. `show_sleep_text_` controls the optional `sleeping...` overlay through the upstream **Sleep Text: Show/Hide** setting.
